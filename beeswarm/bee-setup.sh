...
# Install BEE
cd $HOME
git clone https://`beeswarm_conf.py github_pat`:x-oauth-basic@github.com/lanl/BEE_Private.git \
	-b `beeswarm_conf.py bee_branch` || exit 1
cd BEE_Private
python3 -m venv venv
. venv/bin/activate
...
[task_manager]
listen_port = `beeswarm_conf.py tm_port`
...
[graphdb]
hostname = localhost
dbpass = `beeswarm_conf.py gdb_pass`
bolt_port = 7687
http_port = 7474
https_port = 7473
gdb_image = $GDB_IMG
...
[workflow_manager]
listen_port = `beeswarm_conf.py wfm_port`

[builder]
container_archive = `beeswarm_conf.py ctr_archive`
